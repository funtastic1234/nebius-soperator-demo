...
module "nfs-server" {
  ...
  # Note: Module path adjusted - nfs-server module should be at root modules/ directory
  # If module doesn't exist, ensure nfs.enabled = false in terraform.tfvars
  ...
  # Workaround: Use 0 since nfs.enabled = false and provider::units::from_gib causes parsing issues
  nfs_size             = 0
  nfs_path             = "/home"
