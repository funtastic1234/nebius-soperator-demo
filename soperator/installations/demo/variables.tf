...
variable "o11y_profile" {
  ...
  # Note: Validation commented out due to Terraform limitation - validation cannot reference other variables
  # validation {
  #   condition = (
  ...
  validation {
    condition = alltrue([
      for sm in var.node_local_jail_submounts : (
          # Note: module.resources references not allowed in validation - using hardcoded values
          contains(
            [
              "NETWORK_SSD",
