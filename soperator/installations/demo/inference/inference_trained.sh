#!/bin/bash
#SBATCH --job-name=inference-trained
#SBATCH --output=/opt/demo/inference/inference-trained-%j.out
#SBATCH --error=/opt/demo/inference/inference-trained-%j.err
#SBATCH --nodes=2
...
echo "Starting inference on trained model"
...
RESULTS_DIR=${INFERENCE_DIR}/results/trained
...
if __name__ == "__main__":
    ...
    results_path = "/mnt/inference/results/trained"
    
    if rank == 0:
        print(f"Loading trained model from {model_path}")
    
    # Load model (example - adjust for your model)
...
echo "Inference on trained model completed"
